...
/* FIXME: in msvc, we use __inline, otherwise use __inline. 
 *		  Add #ifdef _MSC_VER check. (chencai)
 */
static __inline void sbc_analyze_four_simd(const int16_t *in, int32_t *out,
...
static __inline void sbc_analyze_eight_simd(const int16_t *in, int32_t *out,
							const FIXED_T *consts)
{
	FIXED_A t1[8];
	FIXED_T t2[8];
...
static __inline void sbc_analyze_4b_4s_simd(struct sbc_encoder_state *state,
...
static __inline void sbc_analyze_4b_8s_simd(struct sbc_encoder_state *state,
		int16_t *x, int32_t *out, int out_stride)
{
	/* Analyze blocks */
...
static __inline void sbc_analyze_1b_8s_simd_even(struct sbc_encoder_state *state,
		int16_t *x, int32_t *out, int out_stride);

static __inline void sbc_analyze_1b_8s_simd_odd(struct sbc_encoder_state *state,
		int16_t *x, int32_t *out, int out_stride)
{
...
static __inline int16_t unaligned16_be(const uint8_t *ptr)
{
	return (int16_t) ((ptr[0] << 8) | ptr[1]);
}

static __inline int16_t unaligned16_le(const uint8_t *ptr)
{
 ...
 * and "big_endian" arguments used with this __inline function as compile
 * time constants.
 */
...
static __inline int sbc_clz(uint32_t x)
{
#ifdef __GNUC__
