				...
				int dualchannel, int snr, int blocks, BOOL msbc)
{
	...
	FILE *fp = NULL;
	//if (strcmp(filename, "-")) {
	//	fp = fopen(filename, "rb");// open(filename, O_RDONLY);
	//	if (fp < 0) {
	...
	len = fread(&au_hdr, 1, sizeof(au_hdr), fp);
	// len = read(fp, &au_hdr, sizeof(au_hdr));
	if (len < (ssize_t) sizeof(au_hdr)) {
		printf("Can't read header from file %s: %s\n",
						filename, strerror(errno));
		goto done;
	}
	...
	if (fread(input, 1, BE_INT(au_hdr.hdr_size) - len, fp) < 0)
		goto done;
	/*if (read(fp, input, BE_INT(au_hdr.hdr_size) - len) < 0)
		goto done;*/
	...
	while (1) {
		...
		size = fread(input, 1, codesize * nframes, fp);
		// size = read(fp, input, codesize * nframes);
		if (size < 0) {
		...
		len = fwrite(output, 1, outp - output, stdout);
		//len = write(_fileno(stdout), output, outp - output);
		if (len != outp - output) {
